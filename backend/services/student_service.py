...
async def validate_student(student_id: str) -> dict:
    ...
    result = (
        ...
        .select("id, student_id, first_name, last_name, middle_initial, program, year_level, photo_url, department_id, email")
    ...
    full_name = build_full_name(student["first_name"], student["last_name"], student.get("middle_initial"))
    ...
    access_token = create_access_token(
        data={
            ...
            "role": "student",
            "full_name": full_name,
            "department_id": str(student.get("department_id")) if student.get("department_id") else None,
        },
    ...
    # Fetch candidate position counts to detect unopposed elections in one query
    election_ids = [e["id"] for e in available_elections]
    unanimous_election_ids: set = set()
    if election_ids:
        cands_res = await (
        ...
        positions_per_election: dict = defaultdict(set)
        candidate_counts: dict = defaultdict(lambda: defaultdict(int))
        for c in (cands_res.data or []):
            eid = c["election_id"]
            pos = c["position"]
            positions_per_election[eid].add(pos)
            candidate_counts[eid][pos] += 1

        # An election is unanimous if it has candidates AND every position has exactly 1
        for eid, positions in positions_per_election.items():
            if positions and all(candidate_counts[eid][pos] == 1 for pos in positions):
                unanimous_election_ids.add(eid)

    # Annotate each election with has_voted / is_unanimous and filter by department
    elections_with_status = []
    for election in available_elections:
        # Cross-department check: only show elections matching student's department
        if election.get("department_id") and str(election["department_id"]) != str(student.get("department_id")):
            continue

        elections_with_status.append({
            **election,
            "has_voted": election["id"] in voted_election_ids,
            "is_unanimous": election["id"] in unanimous_election_ids,
        })
...
async def get_vote_summary(student_id: str, election_id: str) -> dict:
    ...
    student_result = (
        await supabase.table("students")
        .select("id, student_id, first_name, last_name, middle_initial")
        .eq("student_id", student_id)
        .execute()
    )
    if not student_result.data:
    ...
    votes_result = await (
        supabase.table("votes")
        .select(
            "position, candidate_id, created_at, candidates!inner(id, students!inner(first_name, last_name, middle_initial), partylists(name))"
        )
