...
from dependencies.auth import require_adviser
from models import PartylistCreate, CandidateCreate, AdviserChangePassword, AdviserSetEntryPin, AuthUser
...
async def get_elections(
    ...
    """Return all elections visible to the adviser. Scoped by department."""
    all_elecs = await election_service.get_elections()
    if user.role != "super_admin" and user.department_id:
        # Filter elections by department_id
        all_elecs = [e for e in all_elecs if str(e.get("department_id")) == user.department_id]
    return {"data": all_elecs}
...
async def create_partylist(
    ...
    data = await adviser_service.create_partylist(election_id, partylist.name, partylist.logo_url)
    await audit_service.log_action(
        ...
        details={"name": partylist.name, "election_id": election_id, "logo_url": partylist.logo_url},
...
async def update_partylist(
    ...
    data = await adviser_service.update_partylist(partylist_id, partylist.name, partylist.logo_url)
    await audit_service.log_action(
        actor_id=user.id,
        actor_role=user.role,
        ...
        details={"name": partylist.name, "logo_url": partylist.logo_url},
        actor_name=user.full_name,
        actor_username=user.username,
    )
...
async def get_live_results(
    ...
    await election_service.get_election_secure(election_id, user)
...
async def delete_candidate_photo(
    ...
    supabase = await get_async_supabase()
    # Actually clear the photo_url in the database
    await supabase.table("candidates").update({"photo_url": None}).eq("id", candidate_id).execute()
...
async def get_audit_log(
    ...
    # Filter by department_id if not super_admin
    filters = {}
...
async def search_students(
    ...
    return {"data": await adviser_service.search_students(
        query=query, 
        department_id=user.department_id,
        role=user.role
    )}
