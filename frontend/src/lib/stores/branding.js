...
const STORAGE_VERSION = 4; // Bump if DEFAULTS shape changes
...
export function applyBrandingCSS(val) {
	...
	root.style.setProperty('--brand-primary-alpha-5', `rgba(${r}, ${g}, ${b}, 0.05)`);
	root.style.setProperty('--brand-primary-alpha-10', `rgba(${r}, ${g}, ${b}, 0.10)`);
	root.style.setProperty('--brand-primary-alpha-20', `rgba(${r}, ${g}, ${b}, 0.20)`);
	root.style.setProperty('--brand-secondary', val.accentColor);

	// Parse accent once
	const ar = parseInt(val.accentColor.slice(1, 3), 16);
	const ag = parseInt(val.accentColor.slice(3, 5), 16);
	const ab = parseInt(val.accentColor.slice(5, 7), 16);
	root.style.setProperty('--brand-accent-rgb', `${ar}, ${ag}, ${ab}`);
	root.style.setProperty('--brand-accent-alpha-5', `rgba(${ar}, ${ag}, ${ab}, 0.05)`);
	root.style.setProperty('--brand-accent-alpha-10', `rgba(${ar}, ${ag}, ${ab}, 0.10)`);
...
if (browser) {
	...
	branding.subscribe((val) => {
		...
		try {
			sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ _v: STORAGE_VERSION, ...val }));
		} catch {
...
export async function loadBranding() {
	try {
		...
		branding.set({
			...
			showBgAnims: d.show_bg_anims ?? DEFAULTS.showBgAnims,
