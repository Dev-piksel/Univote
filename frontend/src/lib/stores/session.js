 ...
 * @property {string} full_name - Computed display name
...
function createSessionStore() {
	...
	return {
		...
		login(data) {
			...
			const mi = student.middle_initial || null;
			const flattened = {
				...
				last_name: ln,
				middle_initial: mi,
				full_name: mi ? `${fn} ${mi[0].toUpperCase()}. ${ln}` : `${fn} ${ln}`,
				id: student.id,
			...
			if (typeof window !== 'undefined') {
				sessionStorage.setItem(SESSION_KEY, JSON.stringify(flattened));
			}
			set(flattened);
		},
