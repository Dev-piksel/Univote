<script>
	...
	let showBgAnims = $state($branding.showBgAnims);
	let appTheme = $state($branding.theme || 'light');
	...
	let toast = $state({ msg: '', type: 'success', visible: false });
	let toastTimer;
	...
	onMount(async () => {
		try {
			...
			showBgAnims = d.show_bg_anims ?? true;
			appTheme = d.theme || 'light';
		} catch {
	...
	async function handleSave() {
		...
		try {
			...
			showToast('Settings saved! Theme applied across the whole app.', 'success');
			logoFile = null;
		} catch (/** @type {any} */ err) {
		...
		<div class="config-col">
			...
			<div class="settings-card">
				<div class="card-label-row">
					...
					<span class="card-label-text">User Experience</span>
				</div>
				<p class="card-hint">Enable or disable heavy animations to optimize performance on older devices.</p>
				
				<div class="toggle-group">
					<div class="toggle-item">
						<div class="toggle-text">
							<span class="toggle-label">Global Background Animation</span>
							<span class="toggle-desc">Smooth water ripple effects across all portals. (Disable for best performance)</span>
						</div>
						<label class="switch">
							<input type="checkbox" bind:checked={showBgAnims}>
							<span class="slider"></span>
						</label>
					</div>

					<div class="toggle-item mt-6 border-t border-white/5 pt-6">
						<div class="toggle-text">
							<span class="toggle-label">Global System Theme</span>
							<span class="toggle-desc">Force the whole app to use a specific theme by default.</span>
						</div>
						<div class="flex gap-2">
							<button 
								class="px-4 py-2 rounded-xl border text-[10px] font-bold uppercase tracking-widest transition-all {appTheme === 'light' ? 'bg-white text-black border-white' : 'bg-white/5 text-white/40 border-white/10 hover:border-white/20'}"
								onclick={() => appTheme = 'light'}
							>Light</button>
							<button 
								class="px-4 py-2 rounded-xl border text-[10px] font-bold uppercase tracking-widest transition-all {appTheme === 'dark' ? 'bg-white text-black border-white' : 'bg-white/5 text-white/40 border-white/10 hover:border-white/20'}"
								onclick={() => appTheme = 'dark'}
							>Dark</button>
						</div>
					</div>
				</div>
				...
				<div class="mini-sidebar">
					...
					{#each ['Dashboard', 'Elections', 'Voters'] as item, i}
						<div class="mini-sb-item {i === 0 ? 'active' : ''}" style={i === 0 ? `background: color-mix(in srgb, ${primaryColor} 15%, transparent); color: ${primaryColor}; transition: all 0.3s ease;` : 'transition: all 0.3s ease;'}>
							<div class="mini-sb-dot animate-pulse" style={i === 0 ? `background: ${primaryColor};` : ''}></div>
							<span>{item}</span>
						</div>
					{/each}
				</div>
...
<style>
	...
	.toggle-item { display: flex; align-items: center; justify-content: space-between; gap: 1rem; }
	.toggle-text { display: flex; flex-direction: column; gap: 0.15rem; }
	.toggle-label { font-size: 0.8125rem; font-weight: 700; color: var(--text-main); }
	.toggle-desc { font-size: 0.6875rem; color: var(--text-subtle); }
