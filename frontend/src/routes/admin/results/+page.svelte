<script>
	...
	async function loadElections() {
		try {
			...
			dataCache.setElections(res.data || []);
		} catch (err) {
	...
	onMount(() => {
		const unsub = dataCache.subscribe(val => {
			elections = val.elections || [];
		});
		
		async function init() {
			if (elections.length === 0) await loadElections();
		}
		init();
	...
	$effect(() => {
		...
		const isValid = elections.some(e => e.id === id);

		if (id && isValid && id !== connectedElectionId) {
			loadData(id);
			openSSE(id);
		} else if (!id || !isValid) {
			closeSSE();
